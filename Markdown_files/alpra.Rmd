...
```{r}
data <- read_csv(gsub("Markdown_files", "", here("Processed CSV", "alpra_main.csv")))
# add 'logratios' column (update for each pair)
data$logratios <- data$log_alpra_ratio

titlestr = "Alpha-Hydroxyalprazolam/Alprazolam"
savestr = "alpra"
drugname = "Alprazolam"
```
...
logratios = data_DDIfilt$log_alpra_ratio

# alprazolam uses the CYP2C9, CYP3A4/5 enzymes
slow_quant_1 <- 0.02
fast_quant_1 <- 1.00
...
intlabh1 = 1200 #height position for labels
intlabh2 = 1000
intlabh3 = 800
hzoff = 0.1 #offset for labels from vertical line 
...
#  annotate("text",x=UL_gene_1+hzoff, y=intlabh2, hjust=0, size=labsize, label=wrapper(paste("upper", genestr1, round(UL_gene_1,3)),width=labboxwidth2), color=col2) +
  ...
  annotate("text",x=LL_gene_2-3*hzoff, y=intlabh3, hjust=1, size=labsize, label=wrapper(paste("lower", genestr2, round(LL_gene_2,3)),width=labboxwidth2), color=col3) +
...
ggplot(data.frame(logratios), aes(x = logratios)) +
  ...
  geom_vline(xintercept=UL_95, linetype="dashed",color=col1,size=intw) +
  annotate("text",x=UL_95+2*hzoff, y=intlabh1, hjust=0, size=labsize, 
...
boxplot.matrix(as.matrix(listDF), outline = FALSE, notch = TRUE, las=TRUE,
               #names=DDIlabels, 
               yaxt = "n",
               main=substitute(bold(paste("Boxplots of ", log[10], " ", nn, " ratios with drug interactions")), list(nn=titlestr)),
