...
```{r}
data <- read_csv(gsub("Markdown_files", "", here("Processed CSV", "clona_main.csv")))
# add 'logratios' column (update for each pair)
data$logratios <- data$log_clona_ratio

titlestr = "7-Aminoclonazepam/Clonazepam"
savestr = "clona"
drugname = "Clonazepam"
```
...
# clonazepam uses CYP3A4/5 enzymes
#slow_quant_1 <- 0.02
#fast_quant_1 <- 1.00
...
intlabh1 = 300 #height position for labels
intlabh2 = 250
intlabh3 = 370
...
xwdth = 2.1
xmin = xctr-2.2*xwdth # lower x range
...
ggplot(data.frame(logratios), aes(x = logratios)) +
  ...
  geom_density(aes(y=..density..*(bw*ndata)), linewidth=1.0) +
  geom_vline(xintercept=LL_95, linetype="dashed",color=col1,size=intw) +
  annotate("text",x=LL_95+14*hzoff, y=intlabh1, hjust=1, size=labsize, 
...
maxdrugval = quantile(data_DDIfilt[,drugcol], probs=0.96)
