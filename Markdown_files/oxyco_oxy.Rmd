...
```{r}
data <- read_csv(gsub("Markdown_files", "", here("Processed CSV", "oxyco_oxy_main.csv")))
# add 'logratios' column (update for each pair)
data$logratios <- data$log_oxyco_oxy_ratio

titlestr = "Oxymorphone/Oxycodone"
savestr = "oxyco_oxy"
drugname = "Oxycodone"
```
...
logratios = data_DDIfilt$log_oxyco_oxy_ratio

# oxymorphone uses the CYP2D6 enzyme
slow_quant_1 <- 0.054
fast_quant_1 <- 1.00 - 0.018
...
intlabh1 = 3000 #height position for labels
intlabh2 = 2600
intlabh3 = 1400
...
genestr1 = "genetic-based (CYP2D6):"
xctr = median(logratios)
xwdth = 2.8
xmin = xctr-1.6*xwdth # lower x range
...
maxdrugval = quantile(data_DDIfilt[,drugcol], probs=0.98)
