...
```{r}
data <- read_csv(gsub("Markdown_files", "", here("Processed CSV", "imipr_main.csv")))
# add 'logratios' column (update for each pair)
data$logratios <- data$log_imipr_ratio

titlestr = "Desipramine/Imipramine"
savestr = "imipr"
drugname = "Imipramine"
```
...
# imipramine does not have an associated enzyme
slow_quant_1 <- 0.02
fast_quant_1 <- 1.00
...
intlabh1 = 20 #height position for labels
intlabh2 = 1600
intlabh3 = 1400
...
ggplot(data.frame(logratios), aes(x = logratios)) +
  ...
  geom_vline(xintercept=UL_95, linetype="dashed",color=col1,size=intw) +
  annotate("text",x=UL_95+3*hzoff, y=intlabh1, hjust=0, size=labsize, 
