...
```{r}
data <- read_csv(gsub("Markdown_files", "", here("Processed CSV", "bupre_main.csv")))
# add 'logratios' column (update for each pair)
data$logratios <- data$log_bupre_ratio

titlestr = "Norbuprenorphine/Buprenorphine"
savestr = "bupre"
...
# buprenorphine uses the CYP3A4/5 enzymes
#slow_quant_1 <- 0.02
#fast_quant_1 <- 1.00
slow_quant_1 <- (1-0.921)/2
fast_quant_1 <- 0.921+(1-0.921)/2
...
ggplot(data.frame(logratios), aes(x = logratios)) +
  ...
  geom_vline(xintercept=UL_gene_1, linetype="dashed",color=col2,size=intw) +
  annotate("text",x=UL_gene_1+hzoff, y=intlabh2, hjust=0, size=labsize, label=wrapper(paste("upper", genestr1, round(UL_gene_1,3)),width=labboxwidth2), color=col2) +
...
LL_95_2020 <- quantile(data_DDIfilt_2020$logratios, probs = loquant)
UL_95_2020 <- quantile(data_DDIfilt_2020$logratios, probs = hiquant)
LL_95_2021 <- quantile(data_DDIfilt_2021$logratios, probs = loquant)
UL_95_2021 <- quantile(data_DDIfilt_2021$logratios, probs = hiquant)
LL_95_2022 <- quantile(data_DDIfilt_2022$logratios, probs = loquant)
UL_95_2022 <- quantile(data_DDIfilt_2022$logratios, probs = hiquant)
LL_95_2023 <- quantile(data_DDIfilt_2023$logratios, probs = loquant)
UL_95_2023 <- quantile(data_DDIfilt_2023$logratios, probs = hiquant)
LL_95_2024 <- quantile(data_DDIfilt_2024$logratios, probs = loquant)
UL_95_2024 <- quantile(data_DDIfilt_2024$logratios, probs = hiquant)
...
maxdrugval = quantile(data_DDIfilt[,drugcol], probs=0.99)
